...
async def _process_builder_file(
    ...
    # Charge one use for the whole session (first source) + first-charge admin
    # alert, together in the service so a later send can't skip the alert.
    remaining = None
    if not access.is_unlimited(db_user):
        remaining = await access.charge_session_and_notify(message.bot, db_user, session_id)
